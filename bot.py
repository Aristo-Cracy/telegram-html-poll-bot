...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): await update.message.reply_text("Hello! Send me an HTML file with MCQs.")

async def handle_file(update: Update, context: ContextTypes.DEFAULT_TYPE): if update.effective_user.id != OWNER_ID: await update.message.reply_text("Sorry, you are not authorized to use this bot.") return
...
async def send_polls(update: Update, context: ContextTypes.DEFAULT_TYPE): chat_id = update.effective_chat.id questions = pending_polls.pop(chat_id, [])
